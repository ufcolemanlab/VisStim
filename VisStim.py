...
#        windows = {name:Window(self.root, title = name) for name in window_names}
        ...
        self.title_label = tk.Label(self.title_frame, text = "Psycho Commander")
        ...
        self.run_test_grating_button = Button(self.button_frame, "Run Test Grating", self.run_test_grating, (1, 2))
        ...
        self.phase_title = tk.Label(self.phase_title_frame, text = "Canonical Phase Reversal")
        self.phase_title.pack(side = "top")
        
        self.phase_sessions = Entry(self.phase_options_frame, "Sessions", (0,0), default = 5)
        self.phase_reversals = Entry(self.phase_options_frame, "Reversals", (0,1), default = 10)
        self.phase_frequency = Entry(self.phase_options_frame, "Frequency (hz)", (0,2), default = 2)
        self.phase_relaxation = Entry(self.phase_options_frame, "Inter-session length", (0,3), default = 3)
        self.phase_startdelay = Entry(self.phase_options_frame, "Start Delay", (0,4), default = 5)
        
        self.run_reversal_button = Button(self.phase_options_frame, "Run", self.run_phase_reversal, (0, 5))
...
#        #draw gray for Nframs, followed by stim for Nframes
...
#            self.window.flip()
    ...
    def run_phase_reversal(self):
        ...
        for i in range(sessions):
            self.build_stim(self.stim, 'reversal', reversals, frequency = frequency)
    ...
    def build_stim(self, stim, stim_type, length, **kwargs):
        ...
        if stim_type == 'reversal':
            for reversal in range(length):
                ...
                for frame in range(int(self.refresh_rate / kwargs['frequency'])):
                    self.frame_list.append((0.0, 1.0, stim.draw, stim.setPhase, tuple((0.5,))))
